...
let backgroundUrl = "http://giaothong.hochiminhcity.gov.vn/render/ImageHandler.ashx?id=58af8b69bd82540010390c28&t=1734614902105";  // Example dynamic URL
...
canvas.style.backgroundImage = 'url("http://giaothong.hochiminhcity.gov.vn/render/ImageHandler.ashx?id=58af8b69bd82540010390c28&t=1734614902105")';  // Default background

// Function to update the background image once
function updateBackgroundImage() {
    // URL for the dynamic image (camera or any other changing source)
    const dynamicImageUrl = "http://giaothong.hochiminhcity.gov.vn/render/ImageHandler.ashx?id=58af8b69bd82540010390c28&t=1734614902105"; 
...
const pipeGap = 400;
let pipes = [];
let pipeSpeed = 3; // Initial pipe speed
const speedIncrement = 0.25; // Speed increment value
const intervalTime = 1000; // Interval time in milliseconds (1 second)

// Function to update the pipe speed
const increaseSpeed = () => {
    pipeSpeed += speedIncrement;
    // console.log(`Current pipe speed: ${pipeSpeed}`);
};

// Call the increaseSpeed function every second
setInterval(increaseSpeed, intervalTime);


// Words for the pipes (array of words)
const words =  ["",
    "Crowded", "road", "calm", "sky", "slow", "driver", "dusty", "air", 
    "wrong", "turn", "coffee", "thought", "rushing", "students", "scolding", 
    "fear", "cool", "breeze", "late", "clock", "sidewalk", "workout", 
    "fleeting", "calm", "ticking", "time", "behind", "schedule", "construction", 
    "noise", "distant", "laughter", "honking", "horns", "hurried", "steps", 
    "shifting", "shadows", "scattered", "leaves", "muffled", "voices", 
    "bright", "billboards", "quiet", "sighs", "passing", "glances", 
    "fading", "light", "restless", "motion", "endless", "hum", "fleeting", 
    "memories", "whispered", "questions", "rushing", "hours", "waiting", 
    "moments", "hurried", "breath", "missed", "chances"
];
...
birdImage.src = 'assets/2.png'; // Set the path to your image
birdImage.onload = () => {
...
function drawBird() {
    ...
    if (words[wordIndex]) {
        ...
        ctx.font = 'bold 24px "Urbanist", sans-serif'; // Set the font style
        ctx.fillText(words[wordIndex], birdX + 40, birdY + 28); // Draw the text
    
        ctx.lineWidth = 0.5; // Set the stroke width
        // ctx.strokeStyle = 'white'; // Set the stroke color to white
        // ctx.strokeText(words[wordIndex], birdX + birdWidth / 4, birdY + 38); // Stroke the text
    }
    
}
// Load the pipe image
const pipeImage = new Image();
pipeImage.src = 'assets/traffic.png';  // Path to your pipe image
...
function drawScore() {
    ...
    ctx.fillText('CO2 Exhausted: ' + score *87 + ' gram', 10, 30);
}
